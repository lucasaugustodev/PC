"""Inject via SSL_write with proper WS frame + Pomelo Package."""
...
def on_msg(msg, data):
    if msg['type'] == 'send':
        ...
        elif t == 'SSL_GOT': print(f"SSL captured: {p['addr']}")
...
print('\nWaiting for SSL + session...')
for i in range(20):
    time.sleep(1)
    if i % 5 == 4:
        try:
            state = sc.exports_sync.status()
            print(f"  [{i+1}s] ssl={state['ssl']} ready={state['ready']} rid={state['rid']}")
            if state['ssl'] and state['ready']: break
        except Exception as e:
            print(f"  [{i+1}s] error: {e}")

state = sc.exports_sync.status()
if not state['ssl']:
    print("No SSL pointer captured");
...
nextId = lastId + 1
print(f"Ready! lastMsgId={lastId}, starting at {nextId}")
...
for route, body_json in tests:
    ...
    try:
        state = sc.exports_sync.status()
        if not state['ready']:
            print("  Session lost, waiting...")
            for w in range(20):
                time.sleep(1)
                state = sc.exports_sync.status()
                if state['ready']: break
            if not state['ready']:
                print(f"  Skipping {route}"); continue
            if state['rid'] > 0: nextId = state['rid'] + 1
